package com.gestankbratwurst.eproaddons.util.items.display;
...
import com.gestankbratwurst.eproaddons.util.nbtapi.NBTStringList;
...
public class ItemDisplayCompiler extends PacketAdapter implements Function<ItemStack, ItemStack> {
  ...
  public static void addDisplayCompileKey(final String key, final ItemStack item) {
    ItemDisplayCompiler.addDisplayCompileKey(key, new NBTItem(item));
  }

  public static void addDisplayCompileKey(final String key, final NBTItem nbt) {
    final NBTStringList compilerList;
    if (nbt.hasKey(ItemDisplayCompiler.NBT_KEY)) {
      compilerList = nbt.getStringList(ItemDisplayCompiler.NBT_KEY);
    } else {
      compilerList = nbt.createStringList(ItemDisplayCompiler.NBT_KEY);
    }
  ...
  public ItemStack apply(final ItemStack itemStack) {
    ...
    final NBTStringList displayKeys = nbt.getStringList(ItemDisplayCompiler.NBT_KEY);
