...
public class AssetLibrary {
  ...
  private void loadCache() {
    ...
    try {
      final String data = new String(Files.readAllBytes(this.assetCacheFile.toPath()), StandardCharsets.UTF_8);
  ...
  private void iterateAssets() {
    ...
    for (final Model model : Model.values()) {
      ...
      if (!this.itemModelDefaultAssets.containsKey(nmsName)) {
        ...
        final String request = AssetLibrary.jsonGetRequest(jsonUrl);
        if (request == null) {
          return;
        }
  ...
  private static String jsonGetRequest(final String urlQueryString) {
    String json = null;
    try {
  ...
  public void saveCache() {
    ...
    final JsonElement versionObject = gson.toJsonTree(this.itemModelDefaultAssets);
    json.add(AssetLibrary.VERSION, versionObject);

    try {
      Files.write(this.assetCacheFile.toPath(), gson.toJson(json).getBytes(StandardCharsets.UTF_8));
    } catch (final IOException e) {
      e.printStackTrace();
    }
  }
